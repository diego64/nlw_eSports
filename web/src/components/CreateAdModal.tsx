...
export function CreateAdModal() {
  ...
  useEffect(() => {
    axios('http://192.168.0.14:3333/games').then(response => {
  ...
  async function handleCreateAd(event: FormEvent) {
    ...
    try {
      await axios.post(`http://192.168.0.14:3333/games/${data.game}/ads`, {
        name: data.name,
        ...
        <Dialog.Title className='text-3xl font-black'>Publique um anúcio</Dialog.Title>
          ...
          <form onSubmit={handleCreateAd} className="mt-8 flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-2">
              ...
              <Input name="name" id="name"type="text" placeholder='Como te chamam detro do game?'/>
            </div>
